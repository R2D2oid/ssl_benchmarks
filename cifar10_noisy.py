...
logs_root_dir = os.path.join('/scratch/'+user+'/colab/SSL_noisy/')
...
train_base_dataset = NoisyCIFAR10(root='/home/'+user+'/projects/def-jjclark/shared_data/cifar10/data', train=True, download=False, noise_type = noise_type, noise_rate=noise_rate, split_ratio=0.0, transform=train_classifier_transforms)
...
model.load_state_dict(torch.load('/scratch/'+user+'/colab/SSL/'+model_name+'/'+model_name+'_epoch=199.ckpt')['state_dict'])
model.eval()
